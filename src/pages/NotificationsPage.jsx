...
import { db, collection, query, orderBy, limit, onSnapshot, updateDoc, doc, getDoc } from '@/lib/firebase';
...
function NotificationItem({ notif, index }) {
    const [userData, setUserData] = useState(notif.fromUser);

    useEffect(() => {
        // If we have a UID, fetch fresh data. 
        // If not, fall back to snapshot data.
        if (notif.fromUser?.uid) {
            getDoc(doc(db, 'users', notif.fromUser.uid)).then(snap => {
                if (snap.exists()) {
    ...
    // Use fetched data or fallback to snapshot
    const displayUser = userData || notif.fromUser || {};
    const username = displayUser.username || 'Usuario';
    const avatarUrl = displayUser.avatarUrl || displayUser.avatar || "https://github.com/shadcn.png";
